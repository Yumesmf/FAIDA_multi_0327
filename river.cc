...
HLL(CSVDATA csvData_1, CSVDATA csvData_1_hash) {
...
void IND_check(int start)
{
	...
	for (; h2 != hash_hll_2.end(); h2++, rec2++) {
            if (*h1 == *h2) { // hash is same, false positive check.
		auto c1 = rec1->cell.begin();
		auto c2 = rec2->cell.begin();
		//printf("%ld:%ld\n", rec1->cell.size(), rec2->cell.size());

		assert(rec1->cell.size() == 4);
		assert(rec2->cell.size() == 4);
		assert(rec1->cell.size() == rec2->cell.size());
		uint counter;
		for (counter = 0; counter < rec1->cell.size(); counter++) {
		    if (*c1 != *c2) break;
		    else {
			c1++;
			c2++;
		...
		if (counter == rec1->cell.size()) { // all match
...
int main(void)
{
    ...
    csvData_2 = get_record(filepath1, csvData_2, csvData_2_hash).first;
    csvData_2_hash = get_record(filepath1, csvData_2, csvData_2_hash).second;
    ...
    */
    ...
    pair<list<string>, list<string>> p = HLL(csvData_1_hash, csvData_2_hash);
    hash_hll_1 = p.first;
    hash_hll_2 = p.second;
    ...
    for (auto rec = csvData_1.data.begin(); rec != csvData_1.data.end(); rec++) {
